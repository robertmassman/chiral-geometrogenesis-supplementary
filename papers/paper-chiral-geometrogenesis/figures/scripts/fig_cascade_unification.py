...
def create_verification_plot(m_e8_optimal: float):
    ...
    # Output to paper figures directory (parent of scripts/)
    plots_dir = os.path.dirname(os.path.dirname(__file__))
    ...
    # Mark key scales - place labels at top of plot
    for scale, label in [(M_Z, 'M_Z'), (M_GUT, 'M_GUT'), (m_e8_optimal, 'M_E8'), (M_P, 'M_P')]:
        ...
        ax.text(np.log10(scale), 100.5, label, ha='center', fontsize=10)

    ax.set_xlabel('log₁₀(μ/GeV)', fontsize=12)
    ax.set_ylabel(r'$1/\alpha_s$ (inverse strong coupling)', fontsize=12)
    ax.set_title('E₆ → E₈ Cascade Unification', fontsize=14)
    ...
    plt.tight_layout()
    # Save both PNG and PDF for paper
    png_path = os.path.join(plots_dir, 'fig_cascade_unification.png')
    pdf_path = os.path.join(plots_dir, 'fig_cascade_unification.pdf')
...
def create_group_comparison_plot():
    ...
    plt.tight_layout()
    # Save both PNG and PDF for paper
    png_path = os.path.join(plots_dir, 'fig_cascade_group_comparison.png')
    pdf_path = os.path.join(plots_dir, 'fig_cascade_group_comparison.pdf')
    plt.savefig(png_path, dpi=150, bbox_inches='tight')
    plt.savefig(pdf_path, bbox_inches='tight')
    plt.close()

    print(f"✅ Group comparison plot saved to: {png_path}")
    print(f"✅ Group comparison plot saved to: {pdf_path}")
    return pdf_path
