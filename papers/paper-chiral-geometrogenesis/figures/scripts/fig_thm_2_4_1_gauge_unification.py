...
def alpha_inv_running(alpha_inv_0, b, mu_0, mu):
    ...
    d(α⁻¹)/d(ln μ) = -b/(2π)  =>  1/α(μ) = 1/α(μ₀) - (b/2π) * ln(μ/μ₀)

    For asymptotically free theories (b < 0), the minus sign makes 1/α increase
    at higher energy (α decreases). For U(1) with b > 0, 1/α decreases.
    """
...
def main():
    ...
    print("    - α₁⁻¹ decreases with energy (U(1) not asymptotically free)")
    print("    - α₂⁻¹, α₃⁻¹ increase (asymptotically free)")
    print("    - SM couplings approximately converge near M_GUT")
