...
def generate_paper_figure(output_dir=None):
    """Generate phase evolution figure for the paper (PDF format)."""
    ...
    print(f"\nGenerating paper figure in: {output_dir}")

    # Generate the phase evolution plot
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    ...
    plt.savefig(os.path.join(output_dir, 'fig_phase_evolution.pdf'),
                dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'fig_phase_evolution.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: fig_phase_evolution.pdf and fig_phase_evolution.png")
