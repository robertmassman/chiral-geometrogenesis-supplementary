#!/usr/bin/env python3
"""
Tetrahedral Hedgehog Correction Visualization
==============================================

Shows how the anisotropic pressure landscape of the stella octangula induces
angular corrections to the isospin hedgehog direction. Instead of pure n̂ = r̂,
the corrected hedgehog tilts toward high-field regions (R, G, B vertices),
creating ℓ=3 harmonic character in the angular correction.

Physics:
  At each point on the sphere, the tangential gradient of |χ_total| tells us
  which direction has increasing field strength. The hedgehog n̂ tilts toward
  these high-field regions:

    δn̂ = ε · ∇_tang |χ_total| / |χ_total|_max

  where ∇_tang = (∂|χ|/∂θ) ê_θ + (1/sinθ)(∂|χ|/∂φ) ê_φ

4 Interactive Scenes (button toggles):
  1. Pure Hedgehog      — Baseline n̂ = r̂ with modulated shell
  2. Pressure Landscape — Sphere colored by |χ_total| + gradient arrows
  3. Correction Vectors — Side-by-side: faint r̂ vs bold corrected n̂
  4. Full Corrected     — Combined: stella + shell + corrected cones

Verification checks:
  - δn̂ · r̂ ≈ 0 at all points (tangentiality)
  - Correction largest between vertices (steepest gradient)
  - Correction smallest at vertices (local field maxima)
  - Approximate T_d symmetry (invariant under R↔G↔B permutation)
...
Output: verification/plots/tetrahedral_hedgehog_correction.html (.png, .pdf)
"""
...
def main():
    ...
    # Scene descriptions for annotations
    scene_config = {
        'scene1': {
            'text': ("<b>Scene 1: Pure Hedgehog</b> — Baseline n̂ = r̂<br>"
                     "Radial isospin vectors on modulated shell. "
                     "No angular correction applied."),
        },
        'scene2': {
            'text': ("<b>Scene 2: Pressure Landscape</b> — Anisotropy of |χ_total|<br>"
                     "Sphere colored by field magnitude (hot = high). "
                     "Arrows show tangential gradient ∇|χ|."),
        },
        'scene3': {
            'text': ("<b>Scene 3: Correction Vectors</b> — Side-by-side comparison<br>"
                     "Gray: pure r̂ (baseline). "
                     "Colored: corrected n̂_corr (color = correction magnitude |δn̂|)."),
        },
        'scene4': {
            'text': ("<b>Scene 4: Full Corrected Hedgehog</b> — Combined picture<br>"
                     "Shell colored by correction magnitude. "
                     f"Correction strength ε = {EPSILON_CORRECTION}."),
        },
    }
    ...
    # Save outputs
    fig.write_html('plots/tetrahedral_hedgehog_correction.html', include_plotlyjs=True)
    print("Saved: plots/tetrahedral_hedgehog_correction.html (interactive)")

    try:
        fig.write_image('plots/tetrahedral_hedgehog_correction.png',
                        width=1200, height=900, scale=2)
        print("Saved: plots/tetrahedral_hedgehog_correction.png")
    except Exception as e:
        print(f"PNG export skipped ({e})")

    try:
        fig.write_image('plots/tetrahedral_hedgehog_correction.pdf',
                        width=1200, height=900)
        print("Saved: plots/tetrahedral_hedgehog_correction.pdf")
    except Exception as e:
        print(f"PDF export skipped ({e})")

    # Print scene trace counts
    for key in ['scene1', 'scene2', 'scene3', 'scene4']:
        print(f"  {key}: {len(trace_groups[key])} traces")
    print(f"  Total: {n_traces} traces")

    # Run verification
    run_verification(t_plus, rot)
